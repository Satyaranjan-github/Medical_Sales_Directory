...
import { useGetAllMarginsQuery, useLazyGetMarginSuggestionsQuery } from "../api/marginApi";
...
const MarginSelectContent = ({
    ...
    const { data: allMarginsRes, isLoading: loadingAll } = useGetAllMarginsQuery(undefined);
    const [getMarginSuggestions, { isFetching }] = useLazyGetMarginSuggestionsQuery();
    const [searchResults, setSearchResults] = useState<IMargin[] | null>(null);
    const debounceRef = useRef<ReturnType<typeof setTimeout> | null>(null);

    const allMargins: IMargin[] = allMarginsRes?.data || [];
    const activeMargins = searchResults !== null ? searchResults : allMargins;
    ...
    const handleInputChange = (inputValue: string) => {
        ...
        debounceRef.current = setTimeout(async () => {
            const res = await getMarginSuggestions(inputValue.trim());
    ...
    const formattedValue = value
        ? {
              ...
              label: typeof value === "object" ? `${value.title} (${value.value}%)` : value
          }
        : null;
        ...
        <Select<SelectOption, false>
            ...
            onChange={(selected) => {
                ...
                const margin =
                    ...
                    searchResults?.find((m) => m._id === selected.value) ||

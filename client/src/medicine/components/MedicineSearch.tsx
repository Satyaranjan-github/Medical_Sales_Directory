import { ChevronRight, Loader2, Pill, Search, X } from "lucide-react";
...
const MedicineSearch = ({
    ...
    const results: IMedicine[] = data?.data || [];
                ...
                <div className="overflow-y-auto flex-1 p-3 sm:p-4 space-y-2">
                    ...
                    {!query.trim() && (
                        <div className="py-8 px-4 text-center space-y-2">
                            ...
                            <p className="text-xs font-bold text-slate-700">Quick Medicine Search</p>
                            <p className="text-[11px] text-slate-400 max-w-xs mx-auto">
                                Type a medicine name or batch number above to quickly locate and view its detail page.
                            </p>
                        </div>
                    )}
                    ...
                    {query.trim() && !isSearching && results.length === 0 && (
                        <div className="py-8 px-4 text-center space-y-2">
                            ...
                            <p className="text-xs font-bold text-slate-800">No Medicines Found</p>
                            <p className="text-[11px] text-slate-400">
                                No medicine matches <span className="font-bold text-slate-600">"{query}"</span>. Try searching with a different keyword.
                            </p>
                        </div>
                    )}
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="space-y-1">
                                {results.map((medicine) => (
                                    <div
                                        key={medicine._id}
                                        onClick={() => handleSelectMedicine(medicine._id as string)}

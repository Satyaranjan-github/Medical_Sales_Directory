...
const MedicineActionButton = ({ medicineData }: MedicineActionButtonProps) => {
    const [openModal, setOpenModal] = useState(false);
    const { deleteMedicine, restoreMedicine, deleteMedicinePermanently } = useMedicineOperations();

    const isDeleted = medicineData.isDeleted;
        ...
        <>
            <section className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-5 sm:p-6 shadow-xs">
                <div className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                        <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200">Medicine Operations</h4>
                        <p className="text-xs text-slate-400">Modify medicine details or archive/restore directory stock.</p>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        {isDeleted ? (
                            <>
                                <button
                                    type="button"
                                    onClick={() => deleteMedicinePermanently(medicineData._id as string)}
                                ...
                                <button
                                    type="button"
                                    onClick={() => restoreMedicine(medicineData._id as string)}
                            ...
                            <>
                                ...
                                <button
                                    type="button"
                                    onClick={() => deleteMedicine(medicineData._id as string)}
            ...
            {/* UPDATE MODAL */}
            {openModal && <MedicineFormModal setOpenModal={setOpenModal} medicineData={medicineData} />}
        </>
    );
};

import { ChevronRight, Loader2, Search, Tag, X } from "lucide-react";
...
const BrandSearch = ({
    ...
    const [trigger, { data, isFetching, isLoading }] = useLazyGetBrandSuggestionsQuery();
    ...
    const results: IBrand[] = data?.data || [];
            ...
            >
                ...
                <div className="p-4 sm:p-5 border-b border-slate-100 flex items-center gap-3 bg-white sticky top-0 z-10">
                    ...
                    <input
                        ...
                        placeholder="Search brands by label or manufacturer name..."
                ...
                <div className="overflow-y-auto flex-1 p-3 sm:p-4 space-y-2">
                    ...
                    {!query.trim() && (
                        <div className="py-8 px-4 text-center space-y-2">
                            ...
                            <p className="text-xs font-bold text-slate-700">Quick Brand Search</p>
                            <p className="text-[11px] text-slate-400 max-w-xs mx-auto">
                                Type a brand or manufacturer name above to quickly locate and view its detail page.
                            </p>
                        </div>
                    )}
                    ...
                    {query.trim() && !isSearching && results.length === 0 && (
                        <div className="py-8 px-4 text-center space-y-2">
                            ...
                            <p className="text-xs font-bold text-slate-800">No Brands Found</p>
                            <p className="text-[11px] text-slate-400">
                                No brand matches <span className="font-bold text-slate-600">"{query}"</span>. Try searching with a different keyword.
                            </p>
                        </div>
                    )}
                    ...
                    {results.length > 0 && (
                        <div className="space-y-1.5">
                            <p className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400 px-3 py-1">
                                Matching Brands ({results.length})
                            </p>
                            <div className="space-y-1">
                                {results.map((brand) => (
                                    <div
                                        key={brand._id}
                                        onClick={() => handleSelectBrand(brand._id as string)}

...
const BrandActionButtons = ({ brandData }: BrandActionButtonsProps) => {
    const [openModal, setOpenModal] = useState(false);
    const { deleteBrand, deleteBrandPermanently, restoreBrand } = useBrandOperations();

    const isDeleted = brandData.isDeleted;
        ...
        <>
            <section className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-5 sm:p-6 shadow-xs">
                <div className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                        <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200">Brand Operations</h4>
                        <p className="text-xs text-slate-400">Update metadata or restore/archive this brand label.</p>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        {isDeleted ? (
                            <>
                                <button
                                    type="button"
                                    onClick={() => deleteBrandPermanently(brandData._id as string)}
                                ...
                                <button
                                    type="button"
                                    onClick={() => restoreBrand(brandData._id as string)}
                            ...
                            <>
                                ...
                                <button
                                    type="button"
                                    onClick={() => deleteBrand(brandData._id as string)}
            ...
            {/* UPDATE MODAL */}
            {openModal && <BrandFormModal setOpenModal={setOpenModal} brandData={brandData} />}
        </>
    );
};
